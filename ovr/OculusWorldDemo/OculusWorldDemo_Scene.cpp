...
{   
    ...
    String fn = MainFilePath;
    fn.StripExtension();

    Ptr<File>    imageFile = *new SysFile(fn + "_LoadScreen.tga");
    Ptr<Texture> imageTex;
    if (imageFile->IsValid())
        imageTex = *LoadTextureTga(pRender, imageFile);

    Ptr<Model> m = *new Model(Prim_Triangles);        
    m->AddVertex(-10,  0,  -10, Color(255,0,0,255), 0.0f, 0.0f);
    m->AddVertex( 10,  0,  -10, Color(0,255,0,255), 1.0f, 0.0f);
    m->AddVertex( 10,  0,  10, Color(0,0,255,255), 1.0f, 1.0f);
    m->AddVertex(-10,  0,  10, Color(255,255,255,255), 0.0f, 1.0f);
    m->AddTriangle(0,1,2);
    m->AddTriangle(0,2,3);
    if (imageTex)
    {
    ...
    MainScene.World.Add(m);
    MainScene.Models.PushBack(m);
}
